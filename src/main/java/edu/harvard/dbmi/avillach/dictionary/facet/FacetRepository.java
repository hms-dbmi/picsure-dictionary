...
public class FacetRepository {
    ...
    @Autowired
    public FacetRepository(NamedParameterJdbcTemplate template, FacetQueryGenerator generator, FacetMapper mapper) {
    ...
    public Optional<Facet> getFacet(String facetCategory, String facet) {
        String sql =
            """
                ...
                """;
        MapSqlParameterSource params = new MapSqlParameterSource().addValue("facetCategory", facetCategory).addValue("facetName", facet);
        return template.query(sql, params, mapper).stream().findFirst().map(f -> f.withChildren(getFacetChildren(f.category(), f.name())));
    }
    ...
    private List<Facet> getFacetChildren(String facetCategory, String parentFacetName) {
        ...
        MapSqlParameterSource params =
            new MapSqlParameterSource().addValue("facetCategory", facetCategory).addValue("facetName", parentFacetName);
